      ...
      <g class="nodes-layer">
        ...
        >
          ...
          <circle
            ...
            :cx="node.x"
            :cy="node.y"
            :r="isNodeHighlighted(node) ? 22 : 18"
            :fill="isNodeHighlighted(node) ? '#ffe3e3' : getNodeColor(node)"
            :stroke="isNodeHighlighted(node) ? '#ff6b6b' : '#333'"
            :stroke-width="isNodeHighlighted(node) ? 4 : 1.5"
...
function isAttributeHighlighted(node: NetworkNode): boolean {
  ...
  for (const edge of connectedEdges) {
    ...
    if (connectedNode && isNodeHighlighted(connectedNode)) {
      return true;
    }
  }
...
async function downloadAsImage() {
  ...
  try {
    ...
    img.onload = () => {
      ...
      link.download = `network-highlight-${props.highlightedPerfId || 'view'}-${new Date().toISOString().slice(0, 10)}.png`;
